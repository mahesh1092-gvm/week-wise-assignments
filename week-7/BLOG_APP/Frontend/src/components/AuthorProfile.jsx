...
function AuthorProfile() {
  ...
  //calling  function on logout
  const onLogout = async () => {
    //call login route
    await logout();
    //navigating to the login component
    navigate("/login");
  };
    ...
    <div className={pageWrapper}>
      ...
      <div className="bg-white border border-[#e8e8ed] rounded-3xl p-6 mb-8 shadow-sm flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Name */}
          <div>
            <p className="text-sm text-[#676774]">WELCOME back</p>
            <h2 className="text-xl font-semibold text-[#141416]">{currentUser?.firstName}</h2>
          </div>
        </div>
